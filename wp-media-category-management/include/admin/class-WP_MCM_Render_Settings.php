...
if ( !class_exists( 'WP_MCM_Render_Settings' ) ) {
    class WP_MCM_Render_Settings {
        ...
        public function render_settings_sections(
            $wp_mcm_section = WP_MCM_SECTION_GEN,
            $wp_mcm_params = array(),
            $wp_mcm_section_label = '',
            $wp_mcm_button_label = ''
        ) {
            // $this->debugMP('msg',__FUNCTION__.' started.');
            $render_output = '';
                ...
                if ( $settings_params['section'] == $wp_mcm_section ) {
                    switch ( $settings_params['type'] ) {
                        ...
                        case WP_MCM_SETTINGS_TYPE_TEXTAREA:
                            ...
                            break;
                        case WP_MCM_SETTINGS_TYPE_FOLDER_MANAGER:
                            $render_output .= $this->render_settings_folder_manager( $settings_params );
                            break;
                        case WP_MCM_SETTINGS_TYPE_TEXT:
                        case WP_MCM_SETTINGS_TYPE_FILENAME:
                        default:
            ...
            // for class="inside"
            $render_output .= '</div>';
            // for id="wp-mcm-search-settings"
            $render_output .= '</div>';
            // for class="metabox-holder"
            $render_output .= '</div>';
            // for class="postbox-container"
        ...
        public function render_settings_hidden( $text_input = false, $extra_styles = '' ) {
            ...
            $render_output .= $this->render_sprintf(
        ...
        /**
         * Render the settings for folder manager
         * 
         * @since 2.6.0
         * @return html
         */
        public function render_settings_folder_manager( $folder_manager_input = false ) {
            // $this->debugMP('msg',__FUNCTION__.' started.');
            $render_output = '';
            if ( $folder_manager_input == false ) {
                return $render_output;
            }
            return $render_output;
        }
        ...
        public function render_settings_iconlist_options( $icon_input = false ) {
            ...
            foreach ( $icon_urls as $icon_filename => $icon_url ) {
                ...
                $render_output .= $this->render_sprintf(
                    $icon_list_input_template,
                    $checked,
                    $icon_input['name'],
                    $icon_url
                );
                $render_output .= '</li>';
                // for icon_list_class_template
            }
            $render_output .= '</ul>';
            // for ul class="wp-mcm-marker-list"
            // Return output
            //
            return $render_output;
        }
        ...
        function render_settings_iconlist_get_icon_urls( $icon_input = false ) {
            ...
            if ( !isset( $icon_input['icon_object'] ) || $icon_input['icon_object'] == null ) {
                return $icon_urls;
            }
            $wp_mcm_icon_object = $icon_input['icon_object'];
            // e.g. $wp_mcm_social
            $wp_mcm_icon_subdir = $icon_input['icon_subdir'];
            // e.g. 'social-icons/'
            $htmlStr = '';
            $files = array();
            $fqURL = array();
            ...
            //
            if ( isset( $wp_mcm_icon_object->iconselector_files ) && isset( $wp_mcm_icon_object->iconselector_urls ) && $wp_mcm_icon_object->iconselector_files != '' && $wp_mcm_icon_object->iconselector_urls != '' ) {
                ...
                $icon_assets = apply_filters( 'wp_mcm_icon_directories', array(array(
                    'dir' => WP_MCM_UPLOAD_DIR . $icon_input['icon_subdir'],
            ...
            foreach ( $files as $filename => $fqURLIndex ) {
                if ( preg_match( '/\\.(png|gif|jpg)/i', $filename ) > 0 && preg_match( '/shadow\\.(png|gif|jpg)/i', $filename ) <= 0 ) {
         ...
         */
        protected function render_sprintf(
            $template,
            $value_1 = '',
            $value_2 = '',
            $value_3 = '',
